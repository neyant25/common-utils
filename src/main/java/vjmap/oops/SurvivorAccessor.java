...
import vjmap.ClassStats;
import vjmap.utils.FormatUtils;
...
public class SurvivorAccessor {
	...
	public List<ClassStats> caculateHistogram(int excactAge, int minAge) {
		HashMap<sun.jvm.hotspot.oops.Klass, ClassStats> classStatsMap = new HashMap<>(2048, 0.2f);
		...
		while (handle.lessThan(fromTop)) {
			...
			Klass klass = (Klass) obj.getKlass();
			if (klass == null) {
				continue;
			}
			...
			ClassStats stats = HeapUtils.getClassStats((sun.jvm.hotspot.oops.Klass) klass, classStatsMap);
			stats.survivorCount++;
